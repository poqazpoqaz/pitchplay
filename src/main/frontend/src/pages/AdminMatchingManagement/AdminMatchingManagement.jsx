...
const AdminMatchingManagement = () => {
  ...
  useEffect(() => {
    if (dataList.length > 0) {
      ...
      // 경기종류 필터링 (매치타입이 "팀"일 때만 적용)
      if (selectedMatchSize) {
        filteredData = filteredData.filter(data => data.teamSize === selectedMatchSize);
      }
      ...
      if (selectedMonth) {
        ...
        filteredData = filteredData.filter(data => {
          ...
          return date && date.getMonth() + 1 === monthIndex; 
        });
      }
      ...
      <div className={styles["table-pagination-container"]}>
        <Table columCount={5} rowCount={15}>
          ...
          {currentDataList.map((data, index) => (
            <TableRow key={index} onClick={() => handleItemClick(data.id)}>
